	...
	queryContents: tool({
		...
		parameters: z.object({
			...
			category: z
				...
				.describe(
					`内容分类。可选值：${formatCategoriesForTools()}；未指定时传 null`,
				),
			...
			subCategory: z
				...
				.transform((value) => value ?? undefined)
				.describe(
					"二级分类，用于更精细的分类。例如：AI/LLM、前端开发、创业融资、UI设计、职业规划等。可以超出推荐范围自定义生成；未指定时传 null",
				),
			...
			tags: z
				...
				.transform((value) => value ?? undefined)
				.describe(
					`标签数组，用于过滤内容。常用标签示例：${COMMON_TAG_EXAMPLES.join(", ")} 等（可以使用推荐标签之外的其他标签）；未指定时传 null`,
				),

			limit: NullableLimitSchema,
		}),
	}),
